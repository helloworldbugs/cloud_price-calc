...
import rds_aliyun_provider
import rds_tencent_provider
import rds_huawei_provider
...
@app.get("/api/rds/specs")
async def get_rds_specs(country: str = Query("中国"), city: str = Query("")):
    if country not in COUNTRY_CITY_MAP:
        return {"data": {"specs": []}}
    ...
    all_specs = set()
    for city_name, provider_map in target_cities.items():
        for provider_key, fetch_func in [
            ("aliyun", rds_aliyun_provider.get_instance_types),
            ("tencent", rds_tencent_provider.get_instance_types),
            ("huawei", rds_huawei_provider.get_instance_types),
        ]:
            for region_id in _region_ids(provider_map, provider_key):
                try:
                    instances = await fetch_func(region_id)
                    ...
                    logger.exception("Failed to fetch RDS specs: provider=%s region=%s", provider_key, region_id)
...
@app.post("/api/rds/compare")
async def compare_rds_prices(
    country: str = Query(...),
    city: str = Query("全部"),
    cpu: int = Query(0),
    mem: float = Query(0),
    ...
    filter_cpu = cpu if cpu > 0 else None
    filter_mem = int(mem) if mem > 0 else None
    ...
    for city_name, provider_map in target_cities.items():
        ...
        if filter_cpu and filter_mem:
            tasks = []
            for region_id in _region_ids(provider_map, "aliyun"):
                tasks.append(_query_one("阿里云", rds_aliyun_provider.query_price, region_id, filter_cpu, filter_mem, city_name))
            for region_id in _region_ids(provider_map, "tencent"):
                tasks.append(_query_one("腾讯云", rds_tencent_provider.query_price, region_id, filter_cpu, filter_mem, city_name))
            for region_id in _region_ids(provider_map, "huawei"):
                tasks.append(_query_one("华为云", rds_huawei_provider.query_price, region_id, filter_cpu, filter_mem, city_name))
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for r in results:
                if isinstance(r, dict):
                    r["city"] = city_name
            ...
            tasks = []
            for region_id in _region_ids(provider_map, "aliyun"):
                tasks.append(rds_aliyun_provider.query_all_prices(region_id))
            for region_id in _region_ids(provider_map, "tencent"):
                tasks.append(rds_tencent_provider.query_all_prices(region_id))
            for region_id in _region_ids(provider_map, "huawei"):
                tasks.append(rds_huawei_provider.query_all_prices(region_id))
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for r in results:
                if isinstance(r, list):
                    for item in r:
